...
: public object
{
	protected:
		...
		char		**elements;		/* Elements for array */
	public:
				Information		();
...
:public Information
{
	...
	virtual			~Array_storage		();
		void		empty_object		();
		void		set_element		(const int element, const char * string);
	const	char *		get_element		(const int element)	const;

		void		destroy_element		(const int element);
	const	int		exist_element		(const int)		const;
		void		insert_element		(const int, const char *);
...
{
    private:
	const	char		**indices;		/* Index strings */
    public:
				Dictionary		();
	virtual			~Dictionary		();
		void		empty_object		();
	const	char		*get_element		(const int element)	const;
		void		set_element		(const int element, const char *index, const char *string);
		void		destroy_element		(const int element);
	const	int		exist_element		(const char *)		const;
	const	Boolean		destroy			(const dbref);
	const	char *		get_index		(const int)		const;
		void		set_index		(const int, const char *);
	const	Boolean		write			(FILE *f)		const;
	const	Boolean		read			(FILE *f, const int db_version, const int iformat);
...
: public Describable_object
{
	...
	const	char		*odrop;		/* what others see if thing is dropped */
    public:
				Lockable_object		();
	...
	const	Boolean		read			(FILE *f, const int db_version, const int iformat);
		...
		void		set_fail_message	(const char *str);
		void		set_drop_message	(const char *str);
		void		set_succ_message	(const char *str);
		void		set_ofail		(const char *str);
		void		set_osuccess		(const char *str);
		void		set_odrop		(const char *str);
	const	boolexp		*get_key		()			const	{ return key; }
		boolexp		*get_key_for_edit	()			const	{ return key; }
	const	char		*get_fail_message	()			const	{ return fail_message; }
	const	char		*get_drop_message	()			const	{ return drop_message; }
	const	char		*get_succ_message	()			const	{ return succ_message; }
	const	char		*get_ofail		()			const	{ return ofail; }
	const	char		*get_osuccess		()			const	{ return osuccess; }
	const	char		*get_odrop		()			const	{ return odrop; }
};
...
: public Lockable_object
{
	...
	const char 		*get_inherited_element	(const int)	const;
...
: public Massy_object
{
	...
	const	Boolean		read			(FILE *f, const int db_version, const int iformat);
		...
		void		set_race		(const char *r);
		void		set_who_string		(const char *s);
		void		set_money		(const int i)			{ money=i; }

	const	char 	*const	*const get_colour_at	()			const	{ return default_colour_at; }
	const	int		get_pennies		()			const	{ return (pennies); }
	const	dbref		get_controller		()			const	{ return (get_owner()); }
	const	char	*const	get_email_addr		()			const	{ return (email_addr); }
	const	char	*const	get_race		()			const	{ return (race); }
	const	long		get_score		()			const	{ return (score); }
	const	long		get_last_name_change	()			const	{ return (last_name_change); }
	const	char	*const get_who_string		()			const	{ return (who_string); }
...

{
	...
	const	int		look_at_aliases		(const char *string)	const;
#endif /* ALIASES */
	...
	const	char *		get_colour		()			const	{return colour;}
		void		set_colour_at		(const char* const* c)		{colour_at = c;}
	const	char * const * const	get_colour_at		()  	   		const	{return colour_at;}
		void		set_colour_play		(cplay *c)  			{colour_play = c;}
...
#ifdef ALIASES
	const	char		*get_alias		(const int which)	const	{ return (alias[which]); }
	const	int		remove_alias		(const char *string);
	const	int		add_alias		(const char *string);
	const	int		has_alias		(const char *string)	const;
...

{
	...
	const	Boolean		destroy			(const dbref);
		...
		void		set_contents_string	(const char *c);
	const	char		*get_contents_string	()			const	{ return (contents_string); }
...
{
    private:
	const	char		*contents_string;	/* the string shown when the container is looked at and contains something */
