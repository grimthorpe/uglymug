...
void context::do_weapon(const CString& name, const CString& parent_name)
{
	...
	if(!Wizard(get_effective_id()))
	{
		notify_colour(player, player, COLOUR_ERROR_MESSAGES, "Sorry, only wizards can instigate armageddon.");
		return;
	}
	...
	if(parent_name)
	{
		...
		parent = matcher.noisy_match_result();
		if(parent != NOTHING)
			if(Typeof(parent) != TYPE_WEAPON)
			{
				notify_colour(player, player, COLOUR_ERROR_MESSAGES, "The parent must be of type weapon.");
				return;
			}
	}
	else
	...
	db[weapon].set_parent(parent);
	db[weapon].set_location(player);
	db[weapon].set_owner(get_effective_id());
	db[weapon].set_key(TRUE_BOOLEXP);

	/* Set the stats of this weapon to be the same as the parent */
	if(parent != NOTHING)
	{
		db[weapon].set_degradation(db[parent].get_degradation());
		db[weapon].set_damage(db[parent].get_damage());
		db[weapon].set_speed(db[parent].get_speed());
		db[weapon].set_ammunition(db[parent].get_ammunition());
		db[weapon].set_range(db[parent].get_range());
		db[weapon].set_degradation(db[parent].get_degradation());
	}
	PUSH(weapon, player, contents);

	set_return_string (unparse_for_return (*this, weapon));
	if(!in_command())
		notify_colour(player, player, COLOUR_MESSAGES, "Weapon created with id #%d", weapon);
	return_status = COMMAND_SUCC;
...
void context::do_armour(const CString& name, const CString& parent_name)
{
	...
	if(!Wizard(get_effective_id()))
	{
		notify_colour(player, player, COLOUR_ERROR_MESSAGES,  "Sorry, only an UglyMUG Wizard can tailor that well.");
		return;
	}
	...
	if(parent_name)
	{
		...
		parent = matcher.noisy_match_result();
		if(parent != NOTHING)
			if(Typeof(parent) != TYPE_ARMOUR)
			{
				notify_colour(player, player, COLOUR_ERROR_MESSAGES, "The parent must be of type armour.");
				return;
			}
	}
	else
	...
	db[armour].set_parent(parent);
	db[armour].set_location(player);
	db[armour].set_owner(get_effective_id());
	db[armour].set_key(TRUE_BOOLEXP);

	if(parent != NOTHING)
	{
		db[armour].set_degradation(db[parent].get_degradation());
		db[armour].set_protection(db[parent].get_protection());
	}
	PUSH(armour, player, contents);

	set_return_string (unparse_for_return (*this, armour));
	if(!in_command())
		notify_colour(player, player, COLOUR_MESSAGES, "Armour created with id #%d", armour);
	return_status = COMMAND_SUCC;
...
void context::do_ammunition(const CString& name, const CString& parent_name)
{
	...
	if(!Wizard(get_effective_id()))
	{
		notify_colour(player, player, COLOUR_ERROR_MESSAGES, "Sorry, only UO (Ugly Ordinance) can manufacture ammo.");
		return;
	}
	...
	if(parent_name)
	{
		...
		parent = matcher.noisy_match_result();
		if(parent != NOTHING)
			if(Typeof(parent) != TYPE_AMMUNITION)
			{
				notify_colour(player, player, COLOUR_ERROR_MESSAGES, "The parent must be of type ammunition.");
				return;
			}
	}
	else
	...
	db[ammunition].set_parent(parent);
	db[ammunition].set_location(player);
	db[ammunition].set_owner(get_effective_id());
	db[ammunition].set_key(TRUE_BOOLEXP);

	if(parent != NOTHING)
		db[ammunition].set_count(db[parent].get_count());

	PUSH(ammunition, player, contents);

	set_return_string (unparse_for_return (*this, ammunition));
	if(!in_command())
		notify_colour(player, player, COLOUR_MESSAGES, "Ammunition created with id #%d", ammunition);
	return_status = COMMAND_SUCC;
