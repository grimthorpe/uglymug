	...
	FILE	*f)
{
	...
	for(fieldtype=getfieldtype(f); fieldtype != 257; fieldtype=getfieldtype(f))
	{
		...
		default:
			...
			Trace( "Field Type: %d\n", fieldtype);
#else
			log_bug("Something has gone seriously wrong in Weapon::read\nField Type: %d", fieldtype);
#endif /* NEW_LOGGING */
			exit(1);
                }
	}
	...
	FILE	*f)
{
	...
	for(fieldtype=getfieldtype(f); fieldtype != 257; fieldtype=getfieldtype(f))
	{
		...
		default:
			...
			Trace( "Field Type: %d\n", fieldtype);
#else
			log_bug("Something has gone seriously wrong in Armour::read\nField Type: %d", fieldtype);
#endif /* NEW_LOGGING */
			exit(1);
                }
	}
	...
	FILE	*f)
{
	...
	for(fieldtype=getfieldtype(f); fieldtype != 257; fieldtype=getfieldtype(f))
	{
		...
		default:
			...
			Trace( "Field Type: %d\n", fieldtype);
#else
			log_bug("Something has gone seriously wrong in Ammunition::read\nField Type: %d", fieldtype);
#endif /* NEW_LOGGING */
			exit(1);
                }
	}
